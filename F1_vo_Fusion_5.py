...
class Config:
    ...
    OUTPUT_VIDEO = "F1_VO_Fusion_5.mp4"
    OUTPUT_RESULT_IMG = "F1_Final_Fusuion_5_Result.png"
...
class MonocularVO:
    ...
    def process_frame(self, frame):
        ...
        # road_mask: 도로가 흰색(255), 배경이 검은색(0)
        road_mask, _ = self.segmentor.get_adaptive_road_mask(frame_resized)
        
        # [핵심 수정 1] VO를 위한 마스크 반전 (Inverted Mask)
        # 도로 위는 특징점이 없으므로 무시하고, '배경(벽, 펜스)'을 추적합니다.
        # bitwise_not을 통해 도로=0, 배경=255로 만듭니다.
        background_mask = cv2.bitwise_not(road_mask)
        
        # 차량 자체 마스크(static_mask)도 적용하여 차체는 제외
        combined_mask = cv2.bitwise_and(background_mask, self.segmentor.static_mask)

        # 2. 특징점 추출 (배경에서만 추출)
        kp, des = self.orb.detectAndCompute(gray, combined_mask)
        ...
        if self.prev_gray is None:
            ...
            self.prev_kp = kp
            self.prev_des = des
            self.trajectory.append((0.0, 0.0)) # 시작점 추가
        ...
        if des is not None and self.prev_des is not None:
            ...
            if len(good_matches) >= 8:
                ...
                if E is not None:
                    ...
                    # 이동량이 유의미할 때 (노이즈 필터링 0.01)
                    if abs(t[2]) > 0.01 and abs(t[2]) < 1.0:
                        
                        # [핵심 수정 2] 퓨전 로직: Translation(이동)은 VO 그대로, Rotation(회전)만 보정
                        
                        # A. VO의 순수 회전/이동 적용
                        scale = 1.0 # 모노큘러는 스케일 1.0 가정 (속도계가 있다면 그걸 써야 함)
                        
                        # B. 도로 중심점 기반 '추가 회전' (Drift 보정)
                        centroid = self.segmentor.get_road_centroid(road_mask)
                        if centroid is not None:
                            # 화면 중앙 대비 도로 중심의 편차 (-0.5 ~ 0.5)
                            deviation = (centroid[0] - (Config.RESIZE_W / 2)) / Config.RESIZE_W
                            
                            # 편차가 크다면(도로가 휜다면) 강제로 회전값을 섞어줌 (Weighted Average)
                            # R 행렬을 직접 건드리는 건 복잡하므로, 누적된 cur_R 대신
                            # 편법으로 이동 벡터 t의 방향을 살짝 꺾어줍니다.
                            
                            # VO가 생각하는 횡이동(t[0])과 도로가 지시하는 횡이동을 섞음
                            # 도로 인식이 훨씬 신뢰도가 높으므로 가중치를 높게 둠
                            t[0] = t[0] * 0.5 + deviation * 0.5
        ...
        # 시각화: 도로 마스크 (빨강) + 배경 마스크 확인용 (초록 틴트 살짝)
        mask_overlay = cv2.cvtColor(road_mask, cv2.COLOR_GRAY2BGR)
        mask_overlay[:, :, 0] = 0
        mask_overlay[:, :, 1] = 0 
        img_out = cv2.addWeighted(img_out, 0.7, mask_overlay, 0.3, 0)
        
        # 도로 중심점 표시
        centroid = self.segmentor.get_road_centroid(road_mask)
        if centroid:
            cv2.circle(img_out, centroid, 8, (0, 255, 255), -1)
            cv2.line(img_out, (int(Config.RESIZE_W/2), Config.RESIZE_H), centroid, (255, 0, 0), 2)
