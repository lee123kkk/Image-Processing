'''
[프로젝트: 지능형 적응형 VO 시스템]
주행 궤적(Trajectory) 생성 및 정밀 마스킹(Car Profile Masking) 적용 코드 V4

개선 사항:
1. 마스킹 수정: 양쪽 하단(UI 영역)의 높이를 상향 조정 (25% -> 35%)
2. 시각화 버그 수정: 궤적이 화면 밖으로 나가는 문제 해결
   - 경로의 Bounding Box를 계산하여 항상 화면 정중앙에 궤적을 배치하도록 로직 변경
'''
...
OUTPUT_VIDEO = "F1_Trajectory_Result_FixedVis.mp4"
OUTPUT_RESULT_IMG = "F1_Trajectory_Comparison_FixedVis.png"
...
class MonocularVO:
    ...
    def get_mask(self, height, width):
        """
        [수정됨] 양쪽 하단 마스크 높이를 올려서 UI를 확실히 가림
        """
        ...
        # 2. [수정 포인트] 양쪽 하단 구석 마스킹 높이 상향 (0.25 -> 0.35)
        # UI나 자막이 조금 더 높게 올라와 있는 경우를 대비
    ...
    def get_trajectory_map(self):
        """
        [수정됨] 궤적 시각화 함수
        - 기존: 시작점 기준 고정 오프셋 -> 화면 밖으로 나가는 문제 발생
        - 변경: 현재 궤적의 Bounding Box 중심을 계산하여 항상 화면 중앙에 오도록 정렬
        """
        ...
        for i in range(1, len(path)):
            ...
            # (좌표 - 중심점) * 스케일 + 화면반절크기
            # 이렇게 하면 궤적의 중심이 항상 화면의 중심에 옴
            draw_x1 = int((x1 - mid_x) * scale_draw) + center_screen
            draw_y1 = int(-(z1 - mid_z) * scale_draw) + center_screen # Z축 반전 (지도 상 위쪽이 전진)
...
def save_comparison_result(trajectory_points):
    ...
    if len(path) > 0:
        ...
        plt.title("Generated Trajectory (Fixed Vis)")
        plt.legend()
        plt.grid(True)
        plt.axis('equal')
...
def main():
    ...
    while True:
        ...
        cv2.imshow("VO System - Fixed Visualization", combined)
        out.write(combined)
